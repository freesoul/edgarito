...
from edgarito.schemas.providers.edgar.submission import CompanySubmissionsResponse, FilingRecent
...
class EdgarClient:
    ...
    async def get_cik(self, ticker: str, use_cache: bool = True, make_cache: bool = True) -> int:
        """Resolve a ticker to its SEC Central Index Key."""
        normalized_ticker = ticker.strip().upper()
        for company in await self.get_tickers(use_cache=use_cache, make_cache=make_cache):
            if company.ticker.upper() == normalized_ticker:
                return company.cik
        raise ValueError(f"Ticker '{ticker}' was not found in the SEC company ticker list")
    ...
    async def get_all_submissions(self, cik: int, use_cache: bool = True, make_cache: bool = True) -> CompanySubmissionsResponse:
        ...
        first = await self.get_submissions(cik, use_cache=use_cache, make_cache=make_cache)
        ...
        for next_filings_info in first.filings.files:
            ...
            additional_filings = await self.get_submission_additional_filings(next_filings_info.name, use_cache=use_cache, make_cache=make_cache)
    ...
    async def _fetch_json_with_retry_and_cache(
        ...
        if use_cache or make_cache:
            local_filesystem_cached_file_path = f"providers/edgar/{FileSystemCache.path_from_url(url)}"
        ...
        for attempt in range(1, max_attempts + 1):
            async with self._session.get(url, timeout=timeout, headers=headers) as resp:
                ...
                if resp.status in (403, 429) and (
                    "Request Rate Threshold Exceeded" in response_text or resp.status == 429
                ):
                    if attempt == max_attempts:
                        raise RuntimeError(f"SEC rate limit persisted after {max_attempts} attempts: {url}")
                    await asyncio.sleep(threshold_exceeded_delay)
                    continue
                if resp.status == 404:
                    raise FileNotFoundError(f"404 Not Found: {url}")
                if resp.status >= 400:
                    raise RuntimeError(f"SEC request failed with HTTP {resp.status}: {url}")
                try:
                    return json.loads(response_text)
                except json.JSONDecodeError as exc:
                    raise RuntimeError(f"SEC returned an invalid JSON response for {url}") from exc
