...
from edgarito.settings import settings
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        #  Download actions
        elif settings.action == Action.DOWNLOAD:
        ...
        elif settings.action == Action.FACTS:
            if resolved_cik is None:
                raise ValueError("Provide a valid ticker with --ticker or a CIK with --cik")
            async with EDGARLowLevelClient(
                ...
                company_facts = await client.get_company_facts(
                    resolved_cik, use_cache=settings.use_cache, make_cache=settings.make_cache, taxonomy_url=settings.taxonomy_url
                )
                print(company_facts)
