...
class ForwardRevenueEstimateService:
    ...
    async def resolve(
        ...
        *,
        ...
        yahoo_symbol = self._provider_symbol(provider_symbols, "yahoo", normalized_symbol)
        ...
        fallback_reason = "; ".join(
            ...
            if item.reason and item.status
            in {
                ForwardEstimateProviderStatus.UNAVAILABLE,
                ForwardEstimateProviderStatus.FAILED,
            }
    ...
    async def get_revenue_estimates(self, *args, **kwargs) -> ForwardRevenueEstimateResult:
    ...
    async def _call_provider(
        ...
        *,
        ...
        method = next(
            (getattr(client, name, None) for name in method_names if hasattr(client, name)),
            None,
        )
        if method is None:
        ...
        for item in estimates:
            ...
            if existing is None or ForwardRevenueEstimateService._estimate_key(item) > (
                ...
                selected[item.fiscal_year] = item.model_copy(
                    update={"confidence": ForwardRevenueEstimateService._confidence(item)}
                )
    ...
    def _confidence(item: ForwardRevenueEstimate) -> str:
        ...
        confidence = (
            "high"
            if item.source.casefold() == "alphavantage" and analysts is not None and analysts >= 10
            else "medium"
    ...
    def _safe_reason(exc: Exception) -> str:
        ...
        if "unsupported ticker" in lowered or "symbol" in lowered and "not found" in lowered:
